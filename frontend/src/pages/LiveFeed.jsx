  ...
  return (
    <Shell nav>
      <AppHeader
        ...
        right={user ? <button type="button" className="btn btn-outline btn-sm" onClick={() => { dispatch(signOut()); navigate("/login"); }}>Sign Out</button> : <Link to="/login" className="btn btn-primary btn-sm">Log in</Link>}
      />
      <main className="screen screen-nav">
        ...
        {loading ? (
          <p style={{ textAlign: "center", padding: "20px" }}>Loading incidents...</p>
        ) : (
