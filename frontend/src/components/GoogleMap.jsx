...
const MAP_ID = process.env.VITE_GOOGLE_MAPS_MAP_ID || "DEMO_MAP_ID";
...
export function GoogleMap({ incidents = [], large = false }) {
  ...
  useEffect(() => {
    ...
    async function initialiseMap() {
      try {
        ...
        const locations = incidents.map((incident) => ({ ...incident, position: { lat: Number(incident.latitude), lng: Number(incident.longitude) } })).filter((incident) => Number.isFinite(incident.position.lat) && Number.isFinite(incident.position.lng));
        ...
        markers = locations.map((incident) => {
          const marker = new AdvancedMarkerElement({ map, position: incident.position, title: incident.title, content: Object.assign(document.createElement("span"), { className: "map-marker", textContent: incident.type || "📍" }) });
  ...
  return <div ref={elementRef} className={`map-preview google-map ${large ? "map-large" : ""}`} aria-label="Incident locations map" />;
}
