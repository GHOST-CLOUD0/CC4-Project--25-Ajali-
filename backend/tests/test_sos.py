...
def test_sos_alert_created_without_login(client):
    ...
    assert incident["title"] == "SOS - Road Accident"


def test_sos_alert_defaults_without_location_or_description(client):
...
def test_sos_rejects_out_of_range_coordinates(client):
    response = client.post(
        "/api/v1/sos",
        json={"category": "fire", "latitude": -95.0, "longitude": 36.8},
    )
    assert response.status_code == 400
